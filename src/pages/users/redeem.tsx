      ...
      <div className="mt-4 space-y-4">
        <div className="space-y-2 text-sm">
            <div className="flex items-center justify-between font-semibold">
                ...
                <span className="text-yellow-500">{voucher.pointsRequired} Poin</span>
            </div>
            <div className="flex items-center justify-between font-semibold">
                <span className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
        ...
        <Button onClick={() => onRedeem(voucher)} disabled={disabled || voucher.stock === 0} className="w-full cursor-pointer">
          {voucher.stock === 0 ? 'Stok Habis' : (disabled ? 'Menukarkan...' : 'Tukar Poin')}
        </Button>
      </div>
    </CardContent>
  </Card>
...
export default function RedeemPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const results = await Promise.allSettled([
      ...
      axios.get<{ redemptions: RedemptionHistory[] }>("/api/redemptions/me", { headers }),
    ]);
  ...
  const handleConfirmRedeem = async () => {
    ...
    try {
      ...
      if (error && typeof error === "object" && "response" in error) {
        const responseData = (error as { response?: { data?: { msg?: string } } }).response?.data;
      ...
      <div className="flex min-h-screen w-full justify-center bg-slate-50 dark:bg-slate-900">
        <div className="container mx-auto max-w-5xl px-4 py-8">
          <header className="mb-8 flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Tukar Poin</h1>
              <p className="text-slate-500 dark:text-slate-400">Gunakan poin Anda untuk hadiah menarik.</p>
            </div>
          ...
          <section>
            <h2 className="mb-4 text-2xl font-semibold">Katalog Hadiah</h2>
            {vouchers.length === 0 ? (
              <p className="text-center text-slate-500">Belum ada hadiah yang tersedia.</p>
            ) : (
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {vouchers.map((voucher) => (
                  <VoucherCard key={voucher._id} voucher={voucher} onRedeem={handleOpenModal} disabled={isRedeeming} />
                ))}
              </div>
            )}
          </section>
          <section className="mt-12">
            <h2 className="mb-4 text-2xl font-semibold">Riwayat Penukaran Anda</h2>
            <Card>
              <CardContent className="pt-6">
                {history.length === 0 ? (
                  <p className="py-8 text-center text-slate-500">Anda belum pernah menukarkan poin.</p>
                ) : (
                  <div className="space-y-4">
                    {history.map((item) => (
                      <div key={item._id} className="flex items-center justify-between rounded-lg bg-slate-100 p-4 dark:bg-slate-800">
                        <div className="flex min-w-0 items-center gap-4">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">
                              {new Date(item.redeemedAt).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}
                            </p>
                          </div>
                        </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Anda akan menukar{" "}
              <strong className="text-yellow-500">{selectedVoucher?.pointsRequired} poin</strong>{" "}
              untuk mendapatkan{" "}
              <strong className="text-slate-800 dark:text-white">{selectedVoucher?.name}</strong>.
              Lanjutkan?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
